...
% The code is currently written for animal A18070-20-77
clear
clc
close all
...
for ii = 1:numel(D)
    ...
    % input iso
    isostr = {'on', 'off'};
    [isoind, ~] = listdlg('PromptString', {D(ii).name; ' '; 'iso?'}, ...
        'ListString', isostr);
    parameters.iso = isostr{isoind};
    % input lorcaserin
    lorstr = {'on', 'off'};
    [lorind, ~] = listdlg('PromptString', {D(ii).name; ' '; 'lorcaserin?'}, ...
        'ListString', lorstr);
    parameters.lor = lorstr{lorind};
