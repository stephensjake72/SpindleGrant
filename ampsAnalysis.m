...
for ii = 1:numel(D)
    ...
    Amp(ii) = peaks.Lmt.vals(1) - data.procdata.Lmt(1);
    Lf_init(ii) = peaks.Lf.vals(1);
    Lf_s(ii) = mean(peaks.Lf.vals(2:end));
    Fmt_init(ii) = peaks.Fmt.vals(1);
    Fmt_s(ii) = mean(peaks.Fmt.vals(2:end));
    ...
    Cell{ii} = data.parameters.cell(1);
