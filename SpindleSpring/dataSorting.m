...
for ii =  38:41 %1:numel(D)
    ...
    for jj = 1:numel(startTimes)
        ...
        recdata.Lf = Lf(win);
        recdata.Fmt = Fmt(win);
        alpha = act(actwin) - startTimes(jj);
        ...
        stretchind = find(D(ii).name == '_');
        parameters.ID = D(ii).name(1:stretchind(1)-1);
        parameters.cell = D(ii).name(stretchind(2)+1:stretchind(3)-1);
        parameters.aff = 'IA';
        parameters.KT = D(ii).name(stretchind(end)+2);
