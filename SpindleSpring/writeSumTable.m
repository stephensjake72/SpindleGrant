...
source = uigetdir('/Volumes/labs/ting/shared_ting/Jake/Spindle spring data');
savedir = '/Volumes/labs/ting/shared_ting/Jake/Spindle spring data/';
filename = ['summaryTable-' char(datetime('today'))];
D = dir(source);
D = D(3:end);
%%
...
for pp = 1:numel(D)
    ...
    sumTable.Address{pp} = [D(pp).folder filesep D(pp).name];
